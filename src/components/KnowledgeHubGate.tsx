...
export default function KnowledgeHubGate({ children }: { children?: React.ReactNode }) {
  ...
  if (unlocked) {
    return <div id="hub-gated-content">{children}</div>;
  }
      ...
      >
        ...
        >
          ...
          {(status === 'success' || status === 'returning') ? (
            <div className="py-6 animate-[fade-in_0.4s_ease-out]">
              <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full" style={{ background: status === 'returning' ? 'rgba(59, 130, 246, 0.15)' : 'rgba(34, 197, 94, 0.15)' }}>
